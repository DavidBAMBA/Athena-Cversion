...
with open(path_csv, 'w') as csv_file:
    ...
    header = "time,dt,mass,total_E,x1_Mom,x2_Mom,x3_Mom,Gamma,x1_KE,x2_KE,x3_KE,Press,x0_ME,x1_ME,x2_ME,x3_ME,bsq,T00_EM,Gam-p,Gam-c,U_th,U_b\n"
...
gamma = df['Gam-c'].to_numpy()* 10 -1 
U_th  = df['U_th'].to_numpy() * 10
U_b   = df['bsq'].to_numpy()/2 *10

# Crear una figura y un conjunto de subgráficas
fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(6, 8), sharex=True)

ax3.plot(time, gamma, linestyle='-', color='b')
ax3.set_ylabel('<Γ>')
ax3.set_yscale('log')

ax1.plot(time, U_b, linestyle='-', color='r')
ax1.set_ylabel('<U_b>')

ax2.plot(time, U_th, linestyle='-', color='g')
ax2.set_ylabel('<U_th>')
ax2.set_xlabel('Time')
