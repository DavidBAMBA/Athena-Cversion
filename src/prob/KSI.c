    ...
    for (k = ks; k <= ke; k++) {
      for (j = js; j <= je; j++) {
        for (i = is; i <= ie; i++) {
          ...
          pGrid->U[k][j][i].d = 3.0*d0;
          pGrid->U[k][j][i].E = 3.0*d0+p0/Gamma_1;
  ...
  if (pGrid->Nx[2] > 1) {
    ...
    for (k = ks; k <= ke; k++) {
      for (j = js; j <= je; j++) {
        for (i = is; i <= ie; i++) {
          cc_pos(pGrid,i,j,k,&x1,&x2,&x3);
          //Zero net momentum, 3D perturbation with x3 modulation
          v1y = amp*0.25*sin(2.0*PI*nx*x1/lx)*cos(2.0*PI*nz*x3/lz)
                *(1+cos(2.0*PI*ny*x2/ly));
...
/* Enroll history outputs (common to 2D and 3D) */
